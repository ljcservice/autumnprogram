...
public class ReportDispatcher implements Filter
{
	/**
	 * 
	 */
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain arg2) throws IOException, ServletException 
	{
		...
		if (MenuRec.get("MenuRefID").length() > 0)
		{
			...
			if (MenuUrl.length() == 0)
				MenuUrl = "/WebPage/reportView/" + crRef.get("MenuParentID") + "/" + MenuRec.get("MenuRefID") + "/" + MenuRec.get("MenuRefID"); // + "_detail.jsp";
		...
		else
		{
			...
			if (MenuUrl.length() == 0)
				MenuUrl = "/WebPage/reportView/" + MenuRec.get("MenuParentID") + "/" + MenuID + "/" + MenuID; // + "_detail.jsp";
		}
		...
		if (rpt != null)
		{
			...
			o = o == null ? "" : o;
			if("exportWord".equals(o))return ;
	        if (o.length() == 0 || "true".equals(fistSelect))
				request.getRequestDispatcher(MenuUrl+"_detail.jsp").forward(request, response);
			else
