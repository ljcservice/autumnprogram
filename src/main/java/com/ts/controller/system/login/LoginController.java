...
public class LoginController extends BaseController {
			...
			if(session!=null){
				...
				if (user != null) {
					...
					if(mfList!=null && mfList.size()>0){
						List<String> newMfList = new ArrayList<String>(mfList.size());
						for(MenuFun mf :mfList){
							newMfList.add(mf.getFUN_CODE());
						}
						//把用户权限信息放session中
					...
					allmenuList = this.getAttributeMenu(session,user.getUSER_ID(), USERNAME, roleMax,mfList); //菜单缓存
	...
	public List<Menu> getAttributeMenu(Session session,String USER_ID, String USERNAME, Integer roleType,List<MenuFun> mfList) throws Exception{
		List<Menu> allmenuList = new ArrayList<Menu>();
