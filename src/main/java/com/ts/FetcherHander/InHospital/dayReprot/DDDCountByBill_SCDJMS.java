 ...
 * 
 */
public class DDDCountByBill_SCDJMS implements IReportBuilder 
{
	private  Logger logger = LogManager.getLogger(DDDCountByBill_SCDJMS.class);
	...
	public String BuildReportWithCR(String ADate, TCommonRecord crPatInfo, Task AOwner)
	{
		...
		{
			List<TCommonRecord> BillDetail =  this.getInpBillDetail(queryHIS, cr);
			// 病人过程唯一号码 
			String patFuncNo = ADate + "-" + cr.get("patient_id") + "-" + cr.get("visit_id") + "-" + UUID.randomUUID().toString().replace("-", "");
			int    antiCount = 0;
			...
			if(BillDetail != null && BillDetail.size() > 0)
			{
				...
				logger.info("patient_id: " + cr.get("patient_id")+" visit_id: " + cr.get("visit_id")+"的病人 共有" + BillDetail.size() + "账单明细");
				for(int i = 0 ; i < BillDetail.size() ; i++)    //此循环中计算中药费用，西药费用，抗生素相关信息
				{
				    TCommonRecord bill  = BillDetail.get(i);
				    bill.set("is_anti", "0");
				    TCommonRecord staff = getStaff(queryHIS,bill.get("doctor_user"));
				    bill.set("doctor_user",staff.get("emp_no"));
				    if((!"".equals(drDept) && !drDept.equals(bill.get("ordered_by")))
				    		||(!"".equals(drDoct)&& !drDoct.equals(bill.get("doctor_user"))))  
                    {
				    	logger.info("patient_id: " + cr.get("patient_id")+" visit_id: " + cr.get("visit_id") 
				    		+ " ordered_by:" + bill.get("ordered_by") + "  doctor_user:" + bill.get("doctor_user") );
                        if(!is_scd) is_scd = true;
                        /*复制对象 */
                        ...
                        /*设置病人计算信息 */
                        staff = getStaff(queryHIS,drDoct);
                        if(staff != null) rs.set("doctor", staff.get("name"));
                        SetPatInfo(ADate, queryHIS, zl, antiCount,
                                specDDDS, limitDDDS, ordinaryDDDS, ddd,
                                charges, costs, anti_costs, drug_costs,
                                anti_charges, drug_charges, outAntiDDD,
                                outAntiCosts, outAntiCharges, outspecDDDS,
                                outlimitDDDS, outordinaryDDDS, rs,outdrug_costs,outdrug_charges,antiDrugInfo);
                        /* 添加病人信息  */
                        this.rsSplit.add(rs);
                        antiCount = 0;
					...
					if(Config.getParamValue("Drug_In_Order_Chinese").equals(bill.get("item_class")))	//中药费用
					{
					...
					else if(Config.getParamValue("Drug_In_Order").equals(bill.get("item_class")))	//西药费用
					{
					    ...
					    if(DrugUtils.isExternalDrug(bill.get("item_code"), bill.get("item_spec")))
					    {
                            // 外用药应该费用
						...
						if(DrugUtils.isKJDrug(bill.get("item_code")))
						{
							...
							{
								zl.put(bill.get("item_code"), bill.getDouble("amount"));
							}
							else
							{
								zl.put(bill.get("item_code"), zl.get(bill.get("item_code")) + bill.getDouble("amount"));
							}
							// 计算联用数
							if(!billing_date_time.equals(bill.get("billing_date_time")))
							{
								billing_date_time = bill.getDateString("billing_date_time");
								anti.set("anti", bill.get("item_code"));
								antiCount = 1;
							}
							else
							{
								if(!anti.get("anti").equals((bill.get("item_code"))))
								{
									anti.set("anti", bill.get("item_code"));
									antiCount++;
								}
							...
							double  dddValue = DDDUtils.CalcDDD(bill.get("item_code"), bill.get("item_spec"), bill.get("units"), bill.get("Firm_ID"),bill.get("amount"), bill.get("COSTS"));
							bill.set("is_anti", "1");
							bill.set("DDD", dddValue + "");
							String mapKey  =  buildMapKey(bill.get("item_code"), bill.get("item_spec"));
							initializeExternalAntiDrugMap(mapKey,antiDrugInfo,bill);
							...
							if("1".equals(DrugUtils.getDrugLevel(bill.get("item_code"), bill.get("item_spec"))))
							{
								...
								logger.info("存在普通级"+bill.get("item_code")+ "●●●" +  bill.get("item_spec")+ "●●●" + ordinaryDDDS);
							}//特殊级抗菌药
							else if(DrugUtils.isSpecDrug(bill.get("item_code"), bill.get("item_spec")))
							{
								specDDDS += dddValue;
								antiDrugInfo.get(mapKey).set("OUTSPECDDDS", dddValue + antiDrugInfo.get(mapKey).getDouble("OUTSPECDDDS") + "");
								if(isOutAnti)
                                {
								    outspecDDDS += dddValue;
                                }
								logger.info("存在特殊级"+bill.get("item_code")+ "●●●" +  bill.get("item_spec")+ "●●●" + specDDDS);
							}// 限制抗菌药
							else if(DrugUtils.isLimitDrug(bill.get("item_code"), bill.get("item_spec")))
							{
								limitDDDS += dddValue;
								antiDrugInfo.get(mapKey).set("OUTLIMITDDDS", dddValue + antiDrugInfo.get(mapKey).getDouble("OUTLIMITDDDS") + "");
								if(isOutAnti)
                                {
								    outlimitDDDS += dddValue;
                                }
								logger.info("存在限制级"+bill.get("item_code") + "●●●" +  bill.get("item_spec")+ "●●●" + limitDDDS);
							}
			...
			else
			{
			    ...
			    SetPatInfo(ADate, queryHIS, zl, antiCount,
                        ...
                        anti_charges, drug_charges, outAntiDDD,
                        outAntiCosts, outAntiCharges, outspecDDDS,
                        outlimitDDDS, outordinaryDDDS, cr,outdrug_costs,outdrug_charges,antiDrugInfo);
			    //没有任何信息情况
			    this.rsSplit.add(cr);
			}
		}
	...
	private TCommonRecord getStaff(JDBCQueryImpl hisQuery,String doctor){
	    TCommonRecord staffComm =  (TCommonRecord) hisQuery.queryForObject("select * from comm.staff_dict t where t.emp_no  = ? or name = ? ", new String[]{doctor,doctor},  new CommonMapper());
	    return staffComm;
	}
	...
	private String buildMapKey(String drugCode,String drugSpec) {
		...
		List<TCommonRecord> crList = query.query("select DDD_PER_UNIT,DDD_UNIT,Dose_Per_Unit,Dose_Units from Drug_Map where Drug_No_Local = '" + drugCode + "' and Drug_Spec = '" + drugSpec + "' and IS_ANTI = 1", new CommonMapper());
		if (crList.size() > 0) {
	    	TCommonRecord crAntiDrug = crList.get(0);
	    	// 考虑地方医院数据库可能没有 Dose_Per_Unit ， Dose_Units 
	    	String dosePerUnit = crAntiDrug.getDouble("DDD_PER_UNIT") != 0 ? crAntiDrug.get("DDD_PER_UNIT") : crAntiDrug.get("Dose_Per_Unit");
			// 取出基本剂量的单位，可能是克、毫克、g、mg、ml、毫升、万单位、单位、万u、万iu。。。。。。
			String doseUnits    = crAntiDrug.getDouble("DDD_PER_UNIT") != 0 ? crAntiDrug.get("DDD_UNIT") : crAntiDrug.get("Dose_Units").trim();
	 ...
	 * @param billInfo
	 */
	private void initializeExternalAntiDrugMap(String mapKey,Map<String,TCommonRecord> externalAntiDrugInfoMap,TCommonRecord billInfo) {
		if("".equals(mapKey)) return;
		if(!externalAntiDrugInfoMap.containsKey(mapKey)) {
			...
			content.set("OUTANTIAMOUNT", billInfo.getDouble("amount") + content.getDouble("OUTANTIAMOUNT") + "");
			content.set("OUTANTICOSTS", billInfo.getDouble("costs") + content.getDouble("OUTANTICOSTS") + "");
			content.set("OUTANTICHARGES", billInfo.getDouble("charges") + content.getDouble("OUTANTICHARGES") + "");
		}
	}
	/**
     * 将传入的时间单位转换成以天为基础的单位
	 ...
	 * @param rs
	 */
	private void handleOutUseAntiDrug(Map<String,TCommonRecord> externalAntiDrugInfoMap, List<TCommonRecord> orders,TCommonRecord rs) {
		if("close".equals(Config.getParamValue("external_antidrug_administration"))) return; //参数值为close时，直接跳过
		if(externalAntiDrugInfoMap.size() == 0 || orders.size() == 0) {
			return;//无抗菌药的情况直接返回
		}
		logger.info("查询 patient_id: " + orders.get(0).get("patient_id") + ",visit_id:" + orders.get(0).get("visit_id") + ",科室编码："+ orders.get(0).get("FUNCDEPTCODE") + "外用抗菌药");
		...
		String andministratins = Config.getParamValue("external_antidrug_administration");
		String drugFlag = Config.getParamValue("Drug_In_Order");
		//模拟数据库的查询
		for (TCommonRecord data : orders) {
			boolean isExternalAntiDrugFlag = data.get("order_class").equals(drugFlag) //西药
					&& "2".endsWith(data.get("order_status"))//已经执行过的医嘱
					&& "0".endsWith(data.get("billing_attr"))//正常计价的医嘱
					&& (("".equals(data.get("administration"))?false:andministratins.contains(data.get("administration"))) ||
							("".equals(data.get("freq_detail"))?false:andministratins.contains(data.get("freq_detail"))))//配置的外用抗菌药使用途径
					&& antiDrugCodeList.contains(data.get("order_code")); //抗菌药
			if(isExternalAntiDrugFlag) {
				antiDrugList.add(data);
				logger.info("#########patient_id: " + data.get("patient_id") + ",visit_id: " + data.get("visit_id") + ",drug_code: " + data.get("order_code") + " 是外用抗菌药,用药途径为： " +  ("".equals(data.get("administration"))?data.get("freq_detail"):data.get("administration")) + "#######");
			}
		}
		if(antiDrugList.size() > 0) {
			logger.info("查询 patient_id: " + orders.get(0).get("patient_id") + ",visit_id: " + orders.get(0).get("visit_id") + ",科室编码："+ orders.get(0).get("FUNCDEPTCODE") + ",通过医嘱查询到的外用抗菌药有：  "  + antiDrugList.size() + " 个");
			double allExternalCosts = 0,allExternalCharges = 0,allExternalDDD = 0,allExternalSpecDDDs = 0,allExternalLimitDDDs = 0,allExternalOrdingaryDDDs = 0;
			for (String str : externalAntiDrugInfoMap.keySet()) {
				for (TCommonRecord data : antiDrugList) {
					if(str.indexOf(data.get("order_code")) >= 0 ) {
						int amount = 0;
						if(str.split("_")[2].endsWith(data.get("dosage_units"))) {
							amount = (int)(Math.ceil(data.getDouble("dosage")/ Double.parseDouble(str.split("_")[1])) //单次使用个数
									* (DateUtils.daysBetweenTwoDate(data.get("start_date_time"),data.get("stop_date_time")) + 1) //使用天数
									* timeUnitToDay(data.get("freq_interval_unit"),data.get("freq_interval"))//换算成天
									* data.getDouble("freq_counter"));//使用频次
						}else {
							//如果医嘱中的使用单位与药品使用单位不一致时，药品使用个数默认为1
							amount = 1;
						}
						TCommonRecord content =  externalAntiDrugInfoMap.get(str);	
						if(amount > content.getInt("OUTANTIAMOUNT")) {
							allExternalCosts += content.getDouble("OUTANTICOSTS");
							allExternalCharges += content.getDouble("OUTANTICHARGES");
							allExternalDDD += content.getDouble("OUTANTIDDD");
							allExternalSpecDDDs += content.getDouble("OUTSPECDDDS");
							allExternalLimitDDDs += content.getDouble("OUTLIMITDDDS");
							allExternalOrdingaryDDDs += content.getDouble("OUTORDINARYDDDS");
						}
						allExternalCosts += content.getDouble("OUTANTICOSTS")/content.getDouble("OUTANTIAMOUNT") * amount;
						allExternalCharges += content.getDouble("OUTANTICHARGES")/content.getDouble("OUTANTIAMOUNT") * amount;
						allExternalDDD += content.getDouble("OUTANTIDDD")/content.getDouble("OUTANTIAMOUNT") * amount;
						allExternalSpecDDDs += content.getDouble("OUTSPECDDDS")/content.getDouble("OUTANTIAMOUNT") * amount;
						allExternalLimitDDDs += content.getDouble("OUTLIMITDDDS")/content.getDouble("OUTANTIAMOUNT") * amount;
						allExternalOrdingaryDDDs += content.getDouble("OUTORDINARYDDDS")/content.getDouble("OUTANTIAMOUNT") * amount;
					}
				}
			}
			logger.info("#########patient_id: "+ orders.get(0).get("patient_id") + 
					",visit_id:" + orders.get(0).get("visit_id") + 
					",由医嘱查询到的局部用抗菌药costs: " + allExternalCosts + 
					",charges: " + allExternalCharges + 
					",ddd: " + allExternalDDD + 
					",specddds: " + allExternalSpecDDDs  + 
					",limitddds: " + allExternalLimitDDDs + 
					",OrdingaryDDDs: " + allExternalOrdingaryDDDs);
			rs.set("outdrug_costs",  isDoubleNaN(allExternalCosts + rs.getDouble("outdrug_costs")));        	//局部药物费用
	        rs.set("outdrug_charges",isDoubleNaN(allExternalCharges +  rs.getDouble("outdrug_charges")));   	//局部药物实收费用
			rs.set("is_out",    "1");       																	//是否使用外用抗菌药物
	        rs.set("outAntiDDD",isDoubleNaN( allExternalDDD + rs.getDouble("outAntiDDD")));                 	//局部抗菌药物
	        rs.set("outAntiCosts",isDoubleNaN(allExternalCosts + rs.getDouble("outAntiCosts")));            	//局部抗菌药物费用
	        rs.set("outAntiCharges", isDoubleNaN(allExternalCharges +  rs.getDouble("outAntiCharges")) );   	//局部抗菌药实收费用
	        rs.set("outspecDDDS",    isDoubleNaN(allExternalSpecDDDs +  rs.getDouble("outspecDDDS")) );     	//局部特殊级抗生素使用频度
	        rs.set("outlimitDDDS",   isDoubleNaN(allExternalLimitDDDs +  rs.getDouble("outlimitDDDS")) );   	//局部限制级抗生素使用频度
	        rs.set("outordinaryDDDS",isDoubleNaN(allExternalOrdingaryDDDs +  rs.getDouble("outordinaryDDDS"))); //局部普通级抗生素使用频度
	        orders.clear();
	 ...
	 * @return
	 */
	private String  isDoubleNaN(double value ){
    ...
    private void SetPatInfo(String ADate, JDBCQueryImpl queryHIS,
            Map<String, Double> zl, int antiCount,
            double specDDDS, double limitDDDS, double ordinaryDDDS, double ddd,
            double charges, double costs, double anti_costs, double drug_costs,
            double anti_charges, double drug_charges, double outAntiDDD,
            double outAntiCosts, double outAntiCharges, double outspecDDDS,
            double outlimitDDDS, double outordinaryDDDS, TCommonRecord rs,double outdrug_costs,double outdrug_charges,Map<String,TCommonRecord> externalAntiDrugInfoMap)
    {
        ...
        int iEtiologySubmitCount = this.getEtiologySubmitCount(queryHIS, rs,orders);
        rs.set("submit_count",(anti_costs > 0.0 ?Integer.toString(iEtiologySubmitCount):"0"));                 
	...
	public void buildReportMiddleWare(String ADate,JDBCQueryImpl queryHIS) 
	{
		...
		rsSplit  = new ArrayList<TCommonRecord>();
 		String strFields  = "";
 		// 过滤SQLServer关键字
 		if("SQLServer".equals(Config.getTableCofig("MedRec.pat_visit").getDbName()))
 		{
		...
		TCommonRecord where = CaseHistoryHelperUtils.genWhereCR("t.discharge_date_time",CaseHistoryFunction.genRToDate("MedRec.pat_visit", "discharge_date_time", "'" + ADate + "'", "yyyy-mm-dd"),"", ">=", "", "");
		lsWheres.add(where);
		where = CaseHistoryHelperUtils.genWhereCR("t.discharge_date_time",CaseHistoryFunction.genRToDate("MedRec.pat_visit", "discharge_date_time", "'" + DateUtils.getDateAdded(1, ADate) + "'", "yyyy-mm-dd"),"", "<", "", "");
		lsWheres.add(where);
		where = CaseHistoryHelperUtils.genWhereCR("t.patient_id", "m.patient_id", "", "join", "", "");
		lsWheres.add(where);
		where = CaseHistoryHelperUtils.genWhereCR("t.patient_id", "dg.patient_id", "", "join", "", "");
		lsWheres.add(where);
		where = CaseHistoryHelperUtils.genWhereCR("t.visit_id", "dg.visit_id", "", "join", "", "");
		lsWheres.add(where);
		//TODO  2016 年 修改 diagnosis_type 3 -> 1
		where = CaseHistoryHelperUtils.genWhereCR("dg.diagnosis_type", "1", "Char", "=", "", "");
		lsWheres.add(where);
		...
		/* 计算年龄*/
		for(TCommonRecord data:resultSet) {
			data.set("age", CalcuateAgeUtil.calcuateAgeByTwoDates(data.get("date_of_birth"),data.get("discharge_date_time"),"yyyy-MM-dd"));
		}	
		/* 计算天数*/
		for(TCommonRecord data:resultSet) {
			data.set("days", CalcuateAgeUtil.getQuot(data.get("admission_date_time"), data.get("discharge_date_time")) + "");
		}
	 ...
	 * @return
	 */
	public List<TCommonRecord> getInpBillDetail(JDBCQueryImpl query,TCommonRecord cr)
	{
		logger.info("查询 patient_id=" + cr.get("patient_id") + " visit_id=" + cr.get("visit_id") + " 账单明细");
		/* 使用中间层 替换   ----李果*/
		ICaseHistoryHelper  chhr = CaseHistoryFactory.getCaseHistoryHelper();
		String strFields = "*";
//		String strFields = "costs,charges,item_code,item_spec,units,amount,ordered_by," + CaseHistoryFunction.genTrunc("inpbill.Inp_Bill_Detail", "billing_date_time", "billing_date_time") + ",item_class";
		List<TCommonRecord> lsWheres = new ArrayList<TCommonRecord>();
		...
		TCommonRecord order = CaseHistoryHelperUtils.genOrderCR("doctor_user", null);
		lsOrders.add(order);
		order = CaseHistoryHelperUtils.genOrderCR("ordered_by", null);
		lsOrders.add(order);
		List<TCommonRecord> resultData = Collections.emptyList();
		try {
			resultData =  chhr.fetchInpBillDetail2CR(strFields, lsWheres, resultData, lsOrders, query);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return resultData;
...
//            CaseHistoryHelperUtils.genWhereGbkCR("", cr.get(""), "char", "in", "", "");
	 ...
	 * @date	20130809
	 */
	public int getEtiologySubmitCount(JDBCQueryImpl query,TCommonRecord cr, List<TCommonRecord> orders)
	{
		logger.info("查询 patient_id=" + cr.get("patient_id") + " visit_id=" + cr.get("visit_id") + ",过程科室="+ cr.get("FUNCDEPTCODE") + " 病原学送检例数");
		// 中间层替换——李果
		ICaseHistoryHelper  chhr = CaseHistoryFactory.getCaseHistoryHelper();
		List<TCommonRecord> result = Collections.emptyList();
		try 
		{
            ...
            lsWheres.add(where);
            where = CaseHistoryHelperUtils.genWhereCR("doctor_user", cr.get("doctor_user"), "Char", "", "", "");
            lsWheres.add(where);
            List<TCommonRecord> lsOrders = new ArrayList<TCommonRecord>();
            TCommonRecord order = CaseHistoryHelperUtils.genOrderCR("start_date_time", "");
            lsOrders.add(order);
		...
		boolean tranBool    = this.getTransfer(cr.get("doctor"), cr.get("FUNCDEPTCODE"), cr.get("patient_id"), cr.get("visit_id"), cr);
		...
		for(int i = 0; i < result.size(); i++)
		{
            ...
            if(i == 0) 
            {
                ...
                if(!"".equals(endDate))
                {
                    ...
                    if(admiDateTime.getTime() > beginDatetime.getTime()) beginDate = cr.get("admission_date_time");
                }
                DeptDoct  = r.get("doctor_user");
            }
		    if((i+1) == result.size())
		    {
		        ...
		        if(!"".equals(endDate))
		        {
		            ...
		            if(disDateTime.getTime() < endDatetime.getTime()) endDate = cr.get("discharge_Date_time");
		        }
		    }
		    DeptDoct = r.get("doctor_user");
		}
...
//    		    cr.set("DOCTOR","");
        ...
        logger.info("查询 patient_id=" + cr.get("patient_id") + " visit_id=" + cr.get("visit_id") + " 病原学送检例数 :" 
        		+ iCount + " tranbol:" + tranBool + " result:" + result.size() + " doctor_user:" + cr.get("doctor_user"));
		return iCount;
	}
	
	public boolean getTransfer(String doctor,String deptCode , String p_id , String v_id , TCommonRecord cr)
	{
	    boolean ret = false;
	    ICaseHistoryHelper  chhr = CaseHistoryFactory.getCaseHistoryHelper();
	    try
	    {
	    	String strFields = "*";
	        List<TCommonRecord> lsWheres = new ArrayList<TCommonRecord>();
	        TCommonRecord where = CaseHistoryHelperUtils.genWhereCR("PATIENT_ID", p_id, "Char", "", "", "");
	        lsWheres.add(where);
	        where = CaseHistoryHelperUtils.genWhereCR("VISIT_ID", v_id, "Char", "", "", "");
	        lsWheres.add(where);
	        where = CaseHistoryHelperUtils.genWhereCR("DEPT_STAYED", deptCode, "Char", "", "", "");
            lsWheres.add(where);
            where = CaseHistoryHelperUtils.genWhereCR("DOCTOR_IN_CHARGE", doctor, "Char", "", "", "");
            lsWheres.add(where);
	        List<TCommonRecord> list = chhr.fetchTransfer2CR(strFields, lsWheres, null, null, null);
	        // 如果没找记录直接返回
	        if(list == null || list.size() == 0) return ret; 
	        ret = true;
	        TCommonRecord tComm = list.get(0);
	        cr.set("FUNCDEPTBEGINDATE", tComm.getDateTimeString("ADMISSION_DATE_TIME"));
	        cr.set("FUNCDEPTENDDATE", tComm.getDateTimeString("DISCHARGE_DATE_TIME"));
	        String funcDays = "";
        	int days = 0;
        	for (TCommonRecord data : list) {
				days += CalcuateAgeUtil.getQuot(data.getDateTimeString("ADMISSION_DATE_TIME"), data.getDateTimeString("DISCHARGE_DATE_TIME")); 
			}
...
//	        cr.set("DOCTOR", tComm.get("DOCTOR_IN_CHARGE"));
	...
	public String getOperation(JDBCQueryImpl query,TCommonRecord cr)
	{
	    ...
	    try
	    {
	        // 中间层替换
	        ICaseHistoryHelper  chhr = CaseHistoryFactory.getCaseHistoryHelper();
	        List<TCommonRecord> resultList   = chhr.fetchOperation2CR(cr.get("patient_id"), cr.get("visit_id"), query);
	        
	        logger.info("查询 patient_id=" + cr.get("patient_id") + " visit_id=" + cr.get("visit_id") + " 存在" + resultList.size() + "条手术信息");
	 ...
	 * 插入出院患者手术记录
	 */
	public void saveOperationResultSet(List<TCommonRecord> list,TCommonRecord tcr)
	{
		...
		tt.execute(new TransaCallback(tc) 
		{
			...
			public void ExceuteSqlRecord() 
			{
				...
				DDDCountByBill_SCDJMS.this.deleteOldOperation(ph, (TCommonRecord)(this.getTranParm().getObj("tcr")));
				int has_anti = 0;
				List<TCommonRecord> list =(List<TCommonRecord>)this.getTranParm().getObj("list"); 
				for(int j = 0 ; j < list.size() ; j++)
				{
					logger.info("第-----------------"+(j+1)+"---------------共---------------"+list.size()+"手术");
					...
					List<TCommonRecord> lsWheres = new ArrayList<TCommonRecord>();
					TCommonRecord where = CaseHistoryHelperUtils.genWhereCR("order_class", Config.getParamValue("Drug_In_Order"), "Char", "", "", "");
					lsWheres.add(where);
					where = CaseHistoryHelperUtils.genWhereCR("order_status", "2", "", "", "1", "");
					lsWheres.add(where);
					where = CaseHistoryHelperUtils.genWhereCR("order_status", "3", "", "", "1", "or");
					lsWheres.add(where);
					where = CaseHistoryHelperUtils.genWhereCR("patient_id", o.get("patient_id"), "Char", "", "", "");
					lsWheres.add(where);
					where = CaseHistoryHelperUtils.genWhereCR("visit_id", o.get("visit_id"), "Char", "", "", "");
					lsWheres.add(where);
					where = CaseHistoryHelperUtils.genWhereCR("repeat_indicator", "0", "Char", "", "2", "");
					lsWheres.add(where);
					where = CaseHistoryHelperUtils.genWhereCR("start_date_time",CaseHistoryFunction.genRToDate("ordadm.orders", "start_date_time", "'" + DateUtils.getDateAdded(-1, ((TCommonRecord)getTranParm().getObj("tcr")).get("ADate")) + "'", "yyyy-mm-dd"), "", ">=", "2", "");
					lsWheres.add(where);
					where = CaseHistoryHelperUtils.genWhereCR("repeat_indicator", "1", "Char", "", "3", "or");
					lsWheres.add(where);
					where = CaseHistoryHelperUtils.genWhereCR("stop_date_time",CaseHistoryFunction.genRToDate("ordadm.orders", "stop_date_time", "'" + DateUtils.getDateAdded(-1, ((TCommonRecord)getTranParm().getObj("tcr")).get("ADate")) + "'", "yyyy-mm-dd"), "", ">=", "3", "");
					lsWheres.add(where);
					where = CaseHistoryHelperUtils.genWhereCR("start_date_time",CaseHistoryFunction.genRToDate("ordadm.orders", "start_date_time", "'" + DateUtils.getDateAdded(-1, ((TCommonRecord)getTranParm().getObj("tcr")).get("ADate")) + "'", "yyyy-mm-dd"), "", "<=", "3", "");
					lsWheres.add(where);
					try {
						order = chhr.fetchOrders2CR(strFields, lsWheres, null, null, null);
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					Timestamp dateTime = new Timestamp(DateUtils.getDateFromString(o.get("OPERATING_DATE")).getTime());
					...
					dateTime = new Timestamp(DateUtils.getDateFromString(((TCommonRecord)getTranParm().getObj("tcr")).get("ADate")).getTime());
					sqlParams.add(dateTime);
	...
	public void deleteOldOperation(JDBCQueryImpl queryPH,TCommonRecord tcr) 
	{
		logger.info("删除Operation_out旧数据");
	  	queryPH.update("delete Operation_out t where  t.link_date = to_date('"+tcr.get("ADate")+"','yyyy-mm-dd') and patient_id='"+tcr.get("patient_id")+"' and visit_id='"+tcr.get("visit_id")+"'");
	}
	@Override
	...
	public void buildBegin(String ADate, Task AOwner)
	{
		...
		LoggerFileSaveUtil.LogFileSave(logger, "APPLOG\\ddd\\DR_" + ADate + "DDD.log");
	}
	
	private void DeleteRsSplit(JDBCQueryImpl queryIas , String date)
	{
		logger.info("删除ias_pat_info_scd旧数据");
	    queryIas.update("delete ias_pat_info_scd where RPT_DATE = to_date('" + date + "','yyyy-mm-dd')");
	}
	...
	private void saveResultSet(String ADate)
	{
		...
		tt.execute(new TransaCallback(tc) 
		{
			...
			public void ExceuteSqlRecord() 
			{
				...
				/* 删除旧数据 */
				DDDCountByBill_SCDJMS.this.DeleteRsSplit(Jquery,this.getTranParm().get("ADate"));
				int i = 1;
				Map<String,String> tempSqlMapping = new HashMap<String,String>(); 
				for (TCommonRecord cr : rsSplit)
				{
					logger.info("第-----" + i + "-----共-----" + rsSplit.size() + "DDD值");
		    		List<Object> sqlParams = new ArrayList<Object>();
					StringBuffer sql = new StringBuffer();
					sql.append(" insert into ias_pat_info_scd ( RPT_DATE, patient_id, visit_id, species, dept_name, dept_code, age, days, ddd_value,  spec_ddd_value, ");
					sql.append(" limit_ddd_value, doctor, max_drug_aday, ordinaryDDDS, is_anti, identity, charge_type, costs, charges, drug_costs, drug_charges, anti_costs, anti_charges, "
				    ...
				    dateTime = new Timestamp(DateUtils.getDateFromString(cr.getDateTimeString("FUNCDEPTBEGINDATE")).getTime());
					sqlParams.add(dateTime);
					dateTime = new Timestamp(DateUtils.getDateFromString(cr.getDateTimeString("FUNCDEPTENDDATE")).getTime());
					sqlParams.add(dateTime);
				    sqlParams.add(cr.get("FUNCDAYS"));          //过程中住院天数
				    sqlParams.add(cr.get("IS_SCD"));            //转科标示 
				    sqlParams.add(cr.get("PAT_FUNCT_NO"));      //病人过程唯一号码 no = date-pid-vid
				    sqlParams.add(cr.get("IS_ORDERFLAG"));      //病人过程唯一号码 no = date-pid-vid
					...
					String tempKey  = cr.get("patient_id") + "_" + cr.get("visit_id") + "_" + this.getTranParm().get("ADate") + "_" + cr.get("FUNCDEPTCODE") + "_" + cr.get("doctor");
	...
	private void deleteBillDetail(JDBCQueryImpl queryIas , String date) {
		  ...
		  queryIas.update("delete ias_inp_bill_detail where RPT_DATE = to_date('" + date + "','yyyy-mm-dd')");
	}
	...
	private void saveBillDetail(String ADate) {
		...
		tt.execute(new TransaCallback(tc) {
			...
			public void ExceuteSqlRecord() {
				JDBCQueryImpl  Jquery =  DBQueryFactory.getQuery("IAS");
				/* 删除旧数据 */
				DDDCountByBill_SCDJMS.this.deleteBillDetail(Jquery,this.getTranParm().get("ADate"));
				List<Object> sqlParams = new ArrayList<Object>();
				StringBuffer sql = new StringBuffer();
				for (TCommonRecord cr : allBillDetials) {
					sql.append(" insert into IAS_INP_BILL_DETAIL ( PATIENT_ID,VISIT_ID,ITEM_NO,ITEM_CLASS,ITEM_NAME,ITEM_CODE,ITEM_SPEC,AMOUNT,UNITS,");
					sql.append("ORDERED_BY,PERFORMED_BY,COSTS,CHARGES,BILLING_DATE_TIME,OPERATOR_NO,RCPT_NO,ST_DATE,CLASS_ON_INP_RCPT,CLASS_ON_RECKONING,");
					sql.append("SUBJ_CODE,CLASS_ON_MR,SPECIAL_CHARGES,ZFBL,SEND_FLAG,INSUR_TRADE_FEE,INSUR_CLASS_FEE,INSUR_SELF_FEE,INSUR_RCPT_CLASS,");
					sql.append("REFUND_ITEM_NO,PRICE_LIMIT,FREE_LIMIT,DOCTOR_IN_CHARGE,IS_ANTI,DDD_VALUE,RPT_DATE");
					sql.append(") values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
					sqlParams.add(cr.get("patient_id"));
					sqlParams.add(cr.get("visit_id"));
					...
					sqlParams.add(cr.get("COSTS"));
					sqlParams.add(cr.get("CHARGES"));
					Timestamp dateTime = new Timestamp(DateUtils.getDateFromString(cr.get("BILLING_DATE_TIME")).getTime());
					sqlParams.add(dateTime);
					sqlParams.add(cr.get("OPERATOR_NO"));
					sqlParams.add(cr.get("RCPT_NO"));
					dateTime = new Timestamp(DateUtils.getDateFromString(cr.get("ST_DATE")).getTime());
					sqlParams.add(dateTime);
					...
					dateTime = new Timestamp(DateUtils.getDateFromString(getTranParm().get("ADate")).getTime());
					sqlParams.add(dateTime);
