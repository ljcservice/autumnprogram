 ...
 */
@Transactional
public class QDYKDataFetcherNew extends ReportScheduler  {

	/* 评审结果存储 */
	@Resource(name = "reviewResultService")
	private IReviewResultServ reviewResultService;

	public QDYKDataFetcherNew() {
		setDebugLevel(50);
	}
	...
	private TCommonRecord getOUTP_MR(JDBCQueryImpl hisQuery, String patient_id,String MrDoctor, String ADate) {
		...
		where = CaseHistoryHelperUtils.genWhereCR(
				CaseHistoryFunction.genTrunc("outpdoct.outp_mr", "VISIT_DATE", "")
	 ...
	 * @throws Exception
	 */
	public TCommonRecord Data2InsertSub(String id, String DrugCode,
			String DrugName, String DrugType, String Adate) throws Exception {
	...
	@SuppressWarnings("unchecked")
	public TCommonRecord getMainRecord(JDBCQueryImpl hisQuery,
			String PatientID, String VisitID, String OrderNo, String OrderSubNo) {
		...
		List<TCommonRecord> crMain = (List<TCommonRecord>) hisQuery.query(sql,
				new CommonMapper());
		if (crMain.size() == 0) {
	 ...
	 * @param orderType
	 */
	private void InsertDrawException(TCommonRecord crm, String ExcSql,
			String orderType) {
		...
		query.update(sql, new Object[] { id, crm.get("patient_id"), visit,
				getPrevDate(), ExcSql, orderType });
	}
	...
	public boolean canRun() {
		try {
			if (super.canRun()) {
				...
				@SuppressWarnings("unchecked")
				List<TCommonRecord> list = peaasQuery
						.query("select rulecode,rulevalue from ruleparameter where rulecode = 'PEAASDataFetcherTime'",
								new CommonMapper());
				peaasQuery = null;
				if ((list != null) && (list.size() > 0)) {
					try {
						TCommonRecord cr = list.get(0);
						System.out.println("PEAASDataFetcher启动时间为："
								+ cr.get("rulevalue"));
	...
	public void FetchOutpOrders(JDBCQueryImpl hisQuery, String ADate)
	{
        ...
        try {
            ...
            TCommonRecord where = CaseHistoryHelperUtils.genWhereCR(CaseHistoryFunction.genTrunc("outpdoct.OUTP_ORDERS_COSTS", "VISIT_DATE", ""),
                    CaseHistoryFunction.genRToDate("outpdoct.OUTP_ORDERS_COSTS", "VISIT_DATE", "'" + ADate + "'", "yyyy-mm-dd")
                    , "", ">=", "", "");
            lsWheres.add(where);
            where = CaseHistoryHelperUtils.genWhereCR(CaseHistoryFunction.genTrunc("outpdoct.OUTP_ORDERS_COSTS", "VISIT_DATE", ""),
		...
		tt.execute(new TransaCallback(parmComm) {
			...
			public void ExceuteSqlRecord() {
				...
				for (TCommonRecord crs : crsS) {
					...
					// 判断抗菌药
					if (!HasKJ)
						HasKJ = DrugUtils.isKJDrug(crs.get("DRUG_CODE"));
					// 判断注射剂
					if (!HasZS)
						HasZS = DrugUtils.isZSDrug(crs.get("drug_code"),crs.get("DRUG_Spec"));
					// 中药注射剂
					if (!CDZS)
						CDZS = DrugUtils.isCenterDrugZS(crs.get("drug_code"),crs.get("DRUG_Spec"));
				}
				/* 药品种数 */
				...
				sqlParams.add(crm.get("IDENTITY") );                                                       // 病人身份
				sqlParams.add(crm.get("presc_no") ); 
	...
	public void FetchDataOut(JDBCQueryImpl hisQuery, String ADate) {
		...
		if ("SQLServer".equals(Config.getTableCofig("pharmacy.DRUG_PRESC_MASTER").getDbName())) 
		{
			strFields = "CHARGE_TYPE,UNIT_IN_CONTRACT,PRESC_TYPE,PRESC_ATTR,PRESC_SOURCE,"
					  + "REPETITION,COSTS,PAYMENTS,ORDERED_BY,PRESCRIBED_BY,PRESCRIBED_BY,ENTERED_BY,"
					  + "DISPENSING_PROVIDER,PRESC_DATE,PRESC_NO,DISPENSARY,PATIENT_ID,"
					  + "NAME,NAME_PHONETIC,'IDENTITY',VISIT_NO,TSLX ";
		}
		List<TCommonRecord> lsWheres = new ArrayList<TCommonRecord>();
		List<TCommonRecord> lsGroup = new ArrayList<TCommonRecord>();
		List<TCommonRecord> lsOrder = new ArrayList<TCommonRecord>();
	...
	private void FetchDataOutDetail(final TCommonRecord crm,final JDBCQueryImpl hisQuery, final String ADate) {
		...
		tt.execute(new TransaCallback() {
			...
			public void ExceuteSqlRecord() {
				 ...
				 * 增加门诊处置信息
				 */
				List<TCommonRecord> disposal = null;
                try
                {
                    strFields = "*";
                    if ("SQLServer".equals(Config.getTableCofig("outpdoct.outp_orders_costs").getDbName()))
                    {
                        strFields = "'IDENTITY',AMOUNT,ITEM_CODE,ITEM_SPEC,ITEM_NAME,ITEM_CLASS,"
                                + "COSTS,CHARGES,CHARGE_TYPE,PRESC_DATE,PATIENT_ID,ORDERED_BY_DEPT,ORDERED_BY_DOCTOR,UNITS";
                    }
                    String tableNames = "outpdoct.outp_orders_costs";
                    lsWheres.clear();
                    where = CaseHistoryHelperUtils.genWhereCR("PATIENT_ID", crm.get("PATIENT_ID"), "Char", "=", "", "");
                    lsWheres.add(where);
                    where = CaseHistoryHelperUtils.genWhereCR(
                                "PRESC_DATE"
                                , CaseHistoryFunction.genRToDate("outpdoct.outp_orders_costs", "PRESC_DATE", "'" + ADate + "'", "yyyy-MM-dd")
                                , "", ">=", "","");
                    lsWheres.add(where);
                    where = CaseHistoryHelperUtils.genWhereCR(
                                "PRESC_DATE"
                                ,CaseHistoryFunction.genRToDate("outpdoct.outp_orders_costs", "PRESC_DATE", "'" + DateUtils.getDateAdded(1, ADate) + "'", "yyyy-MM-dd")
                                ,"", "<", "", "");
                    lsWheres.add(where);
                    /* 除去西药 */
                    where = CaseHistoryHelperUtils.genWhereCR("item_class",Config.getParamValue("Drug_In_Order"), "Char","!=", "", "");
                    lsWheres.add(where);
                    /* 除去中药  */
                    where = CaseHistoryHelperUtils.genWhereCR("item_class",Config.getParamValue("Drug_In_Order_Chinese"),"Char", "!=", "", "");
                    lsWheres.add(where);
                    // 收费标记，1 ： 为收费
                    where = CaseHistoryHelperUtils.genWhereCR("CHARGE_INDICATOR","1","Char", "=", "", "");
                    lsWheres.add(where);
                    sql = chhr.genSQL(strFields, tableNames, lsWheres, null,null);
                    JDBCQueryImpl hisQuery = DBQueryFactory.getQuery("HIS");
                    disposal = hisQuery.query(sql, new CommonMapper());
                }
                catch (Exception e)
                {
                    throw new RuntimeException("中间层查询错误");
				...
				if (crsS.size() == 0 && disposal.size() == 0) return;
				double allcosts = 0;
				for (TCommonRecord crs : disposal) 
				{
					allcosts += crs.getDouble("costs");
					List<Object> sqlParams = new ArrayList<Object>();
					sql = "insert into  PRESC_DISPOSAL(presc_id,item_code,item_name,item_spec,item_class,amount,costs,charges,identity,charge_type,presc_date,patient_id,units,ordered_by_dept,ordered_by_doctor) "
							+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
					sqlParams.add(id);
					sqlParams.add(crs.get("item_code"));
					sqlParams.add(crs.get("item_name"));
					sqlParams.add(crs.get("item_spec"));
					sqlParams.add(crs.get("item_class"));
					sqlParams.add(crs.get("amount"));
					sqlParams.add(crs.get("costs"));
					sqlParams.add(crs.get("charges"));
					sqlParams.add(crm.get("identity"));
					sqlParams.add(crm.get("charge_type"));
					Timestamp dateTime = new Timestamp(DateUtils.getDateFromString(crs.get("presc_date")).getTime());
					sqlParams.add(dateTime);
					sqlParams.add(crm.get("patient_id"));
					sqlParams.add(crs.get("units"));
					sqlParams.add(crs.get("ordered_by_dept"));
					sqlParams.add(crs.get("ordered_by_doctor"));
					query.update(sql,sqlParams.toArray());
				}
				...
				sql = "insert into PRESC"
						...
						+ "DIAGNOSIS_NAMES, Drug_Count, BaseDrug_Count, HasKJ, HasZS, CDZS,ELJSY,DDRUG,MDrug,FSDrug,YLJSY,GZDrug,DMDrug,ExteDrug,PRESCTYPE,"
						+ "PRESCTYPENAME,PRESCCHECKINFO,PRESCCHECKFLAG,PATIENT_NAME,PATIENT_SEX,PATIENT_BIRTH,CHARGES,DISPENSARY,DISPENSARYNAME,IDENTITY,presc_no,ORDER_TYPE,allcosts,TSLX,ANTIDRUGCOSTS) "
						+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                sqlParams.add(id);                                                                             // ID
				sqlParams.add(crm.get("PATIENT_ID"));                                                            // PATIENT_ID 病人id
				sqlParams.add(patAge);                                                                           // PATIENT_AGE 病人年龄
				...
				sqlParams.add((allcosts + ("".equals(crm.get("Costs")) ? DrugCosts: crm.getDouble("Costs"))) + ""); // 治疗费+药费
				sqlParams.add(crm.get("TSLX") );                                                                    // 青岛特殊字段 处方类型 
