...
import com.ts.util.ReflectHelper;
...
public class PagePlugin implements Interceptor {
	...
	public Object intercept(Invocation ivk) throws Throwable {
		...
		if(ivk.getTarget() instanceof RoutingStatementHandler){
			...
			Configuration c =  (Configuration) ReflectHelper.getValueByFieldName(delegate, "configuration");
			if(c.getDatabaseId()!=null ){
				if("mysql".equals(c.getDatabaseId().toLowerCase())){
					dialect = "mysql";
				}else if("oracle".equals(c.getDatabaseId().toLowerCase())){
					dialect = "oracle";
				}else{
					dialect = c.getDatabaseId();
				}
			}
...
//				pageSql.append(page.getCurrentResult());
	...
	public void setProperties(Properties p) {
		//modify by xsl 20170111 此处对于多数据源来说无效，会覆盖掉dialect，判断改变到intercept方法中实时判断。
