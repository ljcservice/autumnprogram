...
public class MenuService implements MenuManager{
	...
	public List<Menu> listSubMenuByParentId(String parentId) throws Exception {
		//缓存菜单
		List<Menu> list = (List<Menu>) Const.SYS_MENU_MAP.get(Const.SYS_MENU_FATHER_ID+parentId);
		if(list == null){
	...
	public void deleteMenuById(String MENU_ID) throws Exception {
		...
		dao.save("MenuMapper.deleteRoleMfById", pd);//删除角色功能关系
		dao.save("MenuMapper.deleteMenuFunById", pd);//删除菜单功能
		dao.save("MenuMapper.deleteMenuById", pd);//删除菜单及所有下级菜单
	}
	...
	public List<Menu> listValidMenu(String MENU_ID, String USER_ID)	throws Exception {
		...
		for(MenuFun fun:menuList){
			validMenuFunCodes.add(fun.getFUN_CODE().trim());
		}
	 ...
	 * @throws Exception
	 */
	public List<Menu> listValidAllMenuQx(String MENU_ID,List<MenuFun> mfList) throws Exception {
		//查询出用户可用的权限内的所有功能列表
		List<String> validMenuFunCodes = new ArrayList<String>(mfList.size());
		for(MenuFun fun:mfList){
			validMenuFunCodes.add(fun.getFUN_CODE().trim());
		}
	...
	public void reloadMenu(String PARENT_ID) throws Exception {
		...
		if(roleType==0){
			allmenuList = this.listAllMenuQx("0");							//获取所有菜单
		}else{
			User user = Jurisdiction.getCurrentUser();
			//查询用户权限
			List<MenuFun> mfList = (List<MenuFun>)Jurisdiction.getSession().getAttribute(user.getUSERNAME()+Const.SESSION_ROLE_RIGHTS);
			if(mfList==null || mfList.size()==0){
				mfList = menuFunService.listAllMenuFunByUserId(user.getUSER_ID());
			}
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void reloadSubMenuByParentId(String parentId) throws Exception {
