...
public class MenuFunService implements MenuFunManager{
	...
	public List<MenuFun> listAllMenuFunByUserId(String USER_ID) throws Exception {
		...
		if(BasicConst.DATABASE_TYPE==1){
			...
			List<Role> list = roleService.getUserRolesById(USER_ID);
		...
		return (List<MenuFun>) dao.findForList("MenuFunMapper.listAllMenuFunByUserId", pd);
	}
	...
	@SuppressWarnings("unchecked")
	@Deprecated
	public List<String> listAllMenuFunByRoleId(String ROLE_ID) throws Exception {
		return null;//(List<String>) dao.findForList("MenuFunMapper.listAllMenuFunByRoleId", ROLE_ID);
	}
