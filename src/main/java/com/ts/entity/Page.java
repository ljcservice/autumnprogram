...
public class Page {
	...
	//拼接分页 页面及JS函数,且获取父页面的查询条件
	public String getPageStr() {
		StringBuffer sb = new StringBuffer();
		if(totalResult>0){
			sb.append("	<ul class=\"pagination pull-right no-margin\"><input type='hidden' id='currentPage' value='"+this.currentPage+"'>\n");
			if(currentPage==1){
				sb.append("	<li><a>共<font color=red>"+totalResult+"</font>条</a></li>\n");
				sb.append("	<li><input type=\"number\" value=\"\" id=\"toGoPage\" style=\"width:50px;text-align:center;float:left\" placeholder=\"页码\" maxlength='6' onchange='checkPage(this)'/></li>\n");
			...
			if(windowType == 1){
				//本体列表分页使用
				sb.append("     var searchValid = parent.document.getElementById(\"searchForm\");\n");
				sb.append("     url = url + \"&\" +$(searchValid).serialize();\n");
			}else if(windowType == 2){
				//同义词列表分页使用
				sb.append("     var standardId = parent.document.getElementById(\"standardId\");\n");
				sb.append("     url = url + \"&standardId=\"+standardId;\n");
			}
			...
			sb.append("if(isNaN(Number(toPaggeVlue))||toPaggeVlue.indexOf(\".\")!=-1){document.getElementById(\"toGoPage\").value=1;return;}");
			sb.append("nextPage(toPaggeVlue);");
			sb.append("}\n");
			
			//页码输入负数，设置为1
			sb.append("function checkPage(obj){\n");
			sb.append("if(Number($(obj).val())<=0){\n");
			sb.append("$(obj).val(1);\n");
			sb.append("}\n");
			sb.append("}\n");
