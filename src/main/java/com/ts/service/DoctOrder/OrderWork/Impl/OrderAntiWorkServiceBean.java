...
@Service("orderAntiWorkServiceBean")
public class OrderAntiWorkServiceBean implements IOrderWorkService {
	...
	{
		return (List<PageData>) daoph.findForList("PatVisitAntiMapper.patvisitlistPage", page); 
	}
	
	@Override
    public PageData patvisitlistbyRefreshId(PageData pd) throws Exception
    {
        return (PageData) daoph.findForObject("PatVisitAntiMapper.patvisitlistbyRefreshId", pd);
    }

	@Override
	public PageData findByPatient(Page page) throws Exception {
		
		return (PageData) daoph.findForObject("PatVisitAntiMapper.patVisitById", page);
	}
	
    public List<PageData> checkRsDetail(PageData pd) throws Exception {
        return (List<PageData>) daoph.findForList("PatVisitAntiMapper.selectCheckOrdersExcelByNgroupnum", pd);
    }
	
	@Override
	public int updatePatVisitNgroupnum(PageData pd) throws Exception {
		daoph.update("PatVisitAntiMapper.UpdatePatVisitNgroupnum", pd);
		return 1;
	}
	...
	public PageData queryPatVisit(PageData pd) throws Exception {
		return (PageData) daoph.findForObject("PatVisitAntiMapper.queryPatVisit" ,pd);
	}
	...
	public void updateExpertPatVisit(PageData pd) throws Exception {
		daoph.update("PatVisitAntiMapper.updateExpertPatVisit", pd);
	}
	/**
	 * 医嘱报表
	 * @param pd
