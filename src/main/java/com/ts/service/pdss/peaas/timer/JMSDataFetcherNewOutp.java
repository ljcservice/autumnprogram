 ...
 */
@Transactional
public class JMSDataFetcherNewOutp extends ReportScheduler  {

	/* 评审结果存储 */
	@Resource(name = "reviewResultService")
	private IReviewResultServ reviewResultService;

	public JMSDataFetcherNewOutp()
	{
		setDebugLevel(50);
	}
	...
	private boolean getDrugYPAndKl(String drugCode , JDBCQueryImpl hisQuery , String... dfs){
	    String sql = "select * from comm.drug_dict t where t.drug_code = '" + drugCode + "' and drug_indicator = 2";
	    try
	    {
	        @SuppressWarnings ("unchecked")
	        ...
	        if(list != null && list.size() > 0)
	        {
	            ...
	            String drugForm = tComm.get("drug_form");
	            for(String df : dfs){
	                if(df.equals(drugForm.trim())){
	                    return true;
	                }    
	            }
	        }
	...
	private void FetchDataOutDetail(final TCommonRecord crm,
		...
		tt.execute(new TransaCallback() {
			...
			public void ExceuteSqlRecord() {
				...
				boolean ExteDrug = false;
				// 检查中药饮片
				boolean ypDrug = false;
				// 检查中药颗粒
				boolean klDrug = false;
				/* 总费用 */
				float DrugCosts = 0f;
				/* 抗菌药药物费用 */
				...
				for (TCommonRecord crs : crsS) 
				{
					...
					// 中药饮片
					if(!ypDrug){
					    ypDrug = getDrugYPAndKl(drug,hisQuery,new String[]{"饮片"});
					}
					// 中药颗粒
					if(!klDrug){
					    klDrug = getDrugYPAndKl(drug, hisQuery, new String[]{"颗粒剂","免煎剂"});
					}
					/* 基本类型 - 可能不精确 */
					String drugType = DrugUtils.getDrugType(crs.get("DRUG_CODE"), crs.get("DRUG_Spec"),crs.get("ADMINISTRATION"));
					List<String> sqlParams = new ArrayList<String>();
				...
				sql = "insert into PRESC"
						...
						+ "DIAGNOSIS_NAMES, Drug_Count, BaseDrug_Count,ypdrug,kldrug,HasKJ, HasZS, CDZS,ELJSY,DDRUG,MDrug,FSDrug,YLJSY,GZDrug,DMDrug,ExteDrug,PRESCTYPE,"
				...
				sqlParams.add((ypDrug ? "1" : "0"));                                                              // 是否有饮片
                sqlParams.add((klDrug ? "1" : "0"));                                                              // 是否有颗粒
