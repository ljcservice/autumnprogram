...
public class InitPdssCache {
	...
	public void setDrugInteractionInfo() throws Exception {
		...
		page.setTotalPage(pageNum);
		while (pageNum <= page.getTotalPage()) {
			List<TDrugInteractionInfo> list = (List<TDrugInteractionInfo>) dao
					.findForObject("DrugMapper.getDrugInteractionPage", page);
			if (list == null)
				return;
	...
	public void queryDrugPerfom() throws Exception {
		...
		page.setTotalPage(pageNum);
		while (pageNum <= page.getTotalPage()) {
			List<TDrugPerformFreqDict> list = (List<TDrugPerformFreqDict>) dao
					.findForList("DrugMapper.queryDrugPerfomPage", page);
			if (list == null)
				return;
	...
	public void queryTMedicareCatalog(final String DRUG_ID) throws Exception {
		...
		page.setTotalPage(pageNum);
		while (pageNum <= page.getTotalPage()) {
			List<TMedicareCatalog> list = (List<TMedicareCatalog>) dao
					.findForList("DrugMapper.queryTMedicareCatalogPage", page);
			if (list == null)
				return;
