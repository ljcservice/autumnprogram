...
//            TPatOrderDrug[] pods = po.getPatOrderDrugs();
            ...
            // 后面所有的审查结果从缓存中取出来
            // 只有这个循环读取数据库
            for (int i = 0; i < arrDrugs.length; i++)
            {
            	TDrug drugA = arrDrugs[i];
            	for (int j = i + 1; j < arrDrugs.length; j++)
            	{
            		TDrug drugB = arrDrugs[j];
...
//                    }
    ...
    public TDrugSecurityRslt Check(String[] Drugs)
    {
        /* 设置访问数据库代码 */
        this.setQueryCode("PDSS");
        /* 药品组合情况*/
        ...
        for(int i =0 ; i <= drugs.size() ; i++)
        {
            for(int j = i+1 ;j< drugs.size();j++)
            {
                ...
                List<TDrugInteractionInfo> list = QueryUtils.queryDrugInteractionInfo(ingrclassidsA, ingrclassidsB, "", query); 
