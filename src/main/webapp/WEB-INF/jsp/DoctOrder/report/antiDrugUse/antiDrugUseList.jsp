					...
					<div class="row">
						<div class="col-xs-12" >
							<div id="searchDiv"  style="vertical-align:bottom;float: left;padding-top: 4px;padding-bottom: 5px;width: 100%;">
								<form name="searchForm" id="searchForm" action="InHospitalRep/DRANO013.do" method="post" > 
							...
							<thead>
								<tr>
									...
									<th class="center" nowrap>登记患者数</th>
								...
								<c:when test="${not empty antiUses}">
									<c:forEach items="${antiUses}" var="antiUse" varStatus="vs" >
										<tr >
											...
											<td nowrap class="center">${antiUse.patcount}</td>
											<td nowrap class="center">${antiUse.count}</td>
											<td nowrap class="center">
												<a href="javascript:viewDetail('${antiUse.dept_code }','${antiUse.doctor_name }')" >病</a>
											</td> 
										</tr>
									</c:forEach>
								</c:when>
...
function viewDetail(dept_code , doctor_name){
	top.jzts();
	...
	diag.Drag=true;
	diag.Title ="抗菌药越级使用统计-病人";
	diag.URL = "<%=path%>/InHospitalRep/DRANO014.do?ss=false&dept_code=" + dept_code + "&doctor_name=" + doctor_name + "&useType=" + useType + "&beginDate=" + begindate + "&endDate=" + enddate ;    
