									...
									<td style="vertical-align:top;padding-left:2px;padding-right:8px;padding-top:5px;" nowrap>
										操作类型：
									 	<select class="chosen-select form-control" name="OP_TYPE" id="OP_TYPE" data-placeholder="操作类型" style="vertical-align:top;width: 100px;">
											<option value=""></option>
											<option value="" <c:if test="${pd.OP_TYPE==''||pd.OP_TYPE==null}">selected</c:if>>全部</option>
							...
							<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
								<thead>
									<tr>
										...
										<th class="center">术语中文名</th>
										<th class="center">术语标准中文名</th>
										<th class="center">来源名称中文</th>
										<th class="center">来源标准中文名</th>
										<th class="center">术语英文名</th>
												...
												<tr>
													<td class='center' style="width: 30px;"><label >
														<c:if test="${osyn.STATUS=='0'}">
															<input type='checkbox' name='ids' value="${osyn.H_ID }" ontoType="${osyn.ONTO_TYPE }" dn_id="${osyn.DN_ID }" stad_dn_id="${osyn.STAD_DN_ID }" operation="${osyn.OP_TYPE }" id="${osyn.H_ID }" alt="${osyn.dn_chn}" title="${osyn.dn_chn }" class="ace" <c:if test="${oysn.STATUS==0}">style="display:block;"</c:if>/><span class="lbl"></span>
														</c:if>
													...
													<td class='center'><a ondblclick="toDetail()">${osyn.DN_CHN }</a></td>
													<td class='center'>${osyn.STAD_CHN}</td>
													<td class='center'>${osyn.ORG_DN_CHN}</td>
													<td class='center'>${osyn.ORG_STAD_CHN}</td>
