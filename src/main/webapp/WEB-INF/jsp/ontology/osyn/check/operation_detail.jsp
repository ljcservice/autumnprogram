					...
					<div class="row">
						...
						<table class="table table-bordered table-hover">
							...
							<tbody id="standTbody" align="center">
								<tr>
									<td align="right" width="24%">手术名称中文:</td>
									<td width="38%">${pd.ON_CHN }</td>
									<td width="38%">${pd_new.DN_CHN}<c:if test="${pd_new.OP_TYPE <=1}"><span class="edit" onclick="setInput(this,'DN_CHN','${pd_new.DN_CHN}');"/></span></c:if></td>
								</tr>
								<tr>
									<td align="right">手术名称英文:</td>
									<td>${pd.ON_ENG }</td>
									<td>${pd_new.DN_ENG}<c:if test="${pd_new.OP_TYPE <=1}"><span class="edit" onclick="setInput(this,'DN_ENG','${pd_new.DN_ENG}');"/></span></c:if></td>
								</tr>
								<tr>
									<td align="right">手术标准名称:</td>
									<td>${pd.STANDARD_NAME }</td>
									<td>${pd_new.STAND_NAME}</td> 
								</tr>
								<tr>
									...
									<td>
										...
										<c:if test="${pd_new.OP_TYPE <=1}"><span class="edit" onclick="setSelect(this,'SYNO_TYPE');"/></span></c:if>
									</td>
								</tr>
								<tr>
									<td align="right">来源-手术名称中文:</td>
									<td>${pd.ORG_OP_CHN }</td>
								...
								<tr>
									...
									<td><c:if test="${pd.IS_DISABLE ==0 }">否</c:if><c:if test="${pd.IS_DISABLE ==1 }">是</c:if></td>
									<td><c:if test="${pd_new.IS_DISABLE ==0 }">否</c:if><c:if test="${pd_new.IS_DISABLE ==1 }">是</c:if><c:if test="${pd_new.OP_TYPE <=1}"><span class="edit" onclick="setSelect(this,'IS_DISABLE');"/></span></c:if></td>
								</tr>
								<tr>
									<td align="right">停用描述:</td>
									<td>${pd.DESCRIPTION }</td>
									<td>${pd_new.DESCRIPTION}<c:if test="${pd_new.OP_TYPE <=1}"><span class="edit" onclick="setInput(this,'DESCRIPTION','${pd_new.DESCRIPTION}');"/></span></c:if></td>
								</tr>
								<tr>
						...
						<div style="padding: 0px;margin: 4px;vertical-align: middle;text-align: center;">
							<c:if test="${pd_new.STATUS==0}">
								...
								<ts:rights code="OSYN_CHECK_PASS_${RIGHTS}">
									<a class="btn btn-mini btn-primary" onclick="pass('${pd_new.H_ID}',0);">通&nbsp;&nbsp;过</a>
								</ts:rights>
...
function checkData(){
	...
	if(DN_CHN!=null){
		...
		if(!checkName($("#DN_CHN"),'DN_CHN')){
			$(DN_CHN).tips({ side:3, msg:'名称已存在，请更改', bg:'#AE81FF',  time:3   });
			flag = false;
		}
	}
