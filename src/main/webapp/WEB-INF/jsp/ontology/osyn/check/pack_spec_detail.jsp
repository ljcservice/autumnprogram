			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<div id="zhongxin" style="overflow: auto;height: 450px;">
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									
									<td align="left" width="24%">
										<c:if test="${pd_new.OP_TYPE==0 }">新增药品包装规格</c:if>
										<c:if test="${pd_new.OP_TYPE==1 }">修改药品包装规格</c:if>
										<c:if test="${pd_new.OP_TYPE==4 }">停用药品包装规格</c:if>
										：<b/>
									</td>
							...
							<tbody id="standTbody" align="center">
								...
								<tr id="PS_CHN">
									<td align="right">药品包装规格_中文：</td>
									<td>${pd.PS_CHN}</td>
									<td>${pd_new.DN_CHN}<c:if test="${pd_new.OP_TYPE<=1 }"><span class="edit" onclick="setInput(this,'DN_CHN','${pd_new.DN_CHN}');"></span></c:if></td>
								</tr>
								<tr id="PS_ENG">
									<td align="right">药品包装规格_英文：</td>
									<td>${pd.PS_ENG}</td>
									<td>${pd_new.DN_ENG}<c:if test="${pd_new.OP_TYPE<=1 }"><span class="edit" onclick="setInput(this,'DN_ENG','${pd_new.DN_ENG}');"></span></c:if></td>
								</tr>
								<tr id="STAD_PS_CHN">
									<td align="right">药品标准包装规格_中文：</td>
									<td>${pd.STAD_PS_CHN }</td>
									<td>${pd_new.STAD_CHN}</td>
								</tr>
								<tr id="STAD_PS_ENG">
									<td align="right">药品标准包装规格_英文：</td>
									<td>${pd.STAD_PS_ENG }</td>
									<td>${pd_new.STAD_ENG}</td>
								</tr>
								<tr id="STAD_PS_VALUE_CHN">
									<td align="right">药品标准包装规格数值_中文：</td>
									<td>${pd.STAD_PS_VALUE_CHN }</td>
									<td>${pd_new.STAD_PACK_SPEC_CHN}<c:if test="${pd_new.OP_TYPE<=1 }"><span class="edit" onclick="setInput(this,'STAD_PACK_SPEC_CHN','${pd_new.STAD_PACK_SPEC_CHN}');"></span></c:if></td>
								</tr>
								<tr id="STAD_PS_VALUE_ENG">
									<td align="right">药品标准包装规格数值_英文：</td>
									<td>${pd.STAD_PS_VALUE_ENG }</td>
									<td>${pd_new.STAD_PACK_SPEC_ENG}<c:if test="${pd_new.OP_TYPE<=1 }"><span class="edit" onclick="setInput(this,'STAD_PACK_SPEC_ENG','${pd_new.STAD_PACK_SPEC_ENG}');"></span></c:if></td>
								</tr>
								<tr id="STAD_PS_UNIT_CHN">
									<td align="right">药品标准包装规格单位_中文：</td>
									<td>${pd.STAD_PS_UNIT_CHN }</td>
									<td>${pd_new.STAD_PACK_SPEC_UNIT_CHN}</td>
								</tr>
								<tr id="STAD_PS_UNIT_ENG">
									<td align="right">药品标准包装规格单位_英文：</td>
									<td>${pd.STAD_PS_UNIT_ENG }</td>
									<td>${pd_new.STAD_PACK_SPEC_UNIT_ENG} </td>
								</tr>
								<tr id="ORG_PS_CHN">
									<td align="right">来源-药品包装规格_中文：</td>
									<td>${pd.ORG_PS_CHN }</td>
								...
								<tr id="ORG_PS_ENG">
									<td align="right">来源-药品包装规格_英文：</td>
									<td>${pd.ORG_PS_ENG }</td>
								...
								<tr id="ORG_STAD_PS_CHN">
									<td align="right">来源-药品标准包装规格_中文：</td>
									<td>${pd.ORG_STAD_PS_CHN }</td>
									<td>${pd_new.ORG_STAD_CHN}</td>
								</tr>
								<tr id="ORG_STAD_PS_ENG">
									<td align="right">来源-药品标准包装规格_英文：</td>
									<td>${pd.ORG_STAD_PS_ENG }</td>
								...
								<tr>
									...
									<td colspan="2"><input type="text" name="CHECK_MEMO" value="${pd_new.CHECK_MEMO }" style="width: 90%;"/></td>
								</tr>
								<tr id="showConsol">
...
function checkData(){
	...
	if(DN_CHN!=null){
		if(DN_CHN.value ==null || DN_CHN.value=="" ){
			$(DN_CHN).tips({ side:3, msg:'请输入包装规格中文', bg:'#AE81FF',  time:3   });
			flag = false;
		}
