						...
						<div class="col-xs-12">
								<form action="osyn/saveDiagOsyn.do" name="diagOsynForm" id="diagOsynForm" method="post">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<tr>
											<td style="width:130px;text-align: right;padding-top: 13px;">给药途径名称_中文：</td>
											<td><input type="text" name="DN_CHN" id="DN_CHN" maxlength="255" onblur="checkName(this,'DN_CHN');" value="${pd.CHN_NAME }" style="width:98%;"/></td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源-给药途径名称_中文：</td>
											<td><input type="text" name="ORG_DN_CHN" id="ORG_DN_CHN"  value="${pd.ORG_CHN_NAME }"  maxlength="255" style="width:98%;"/></td>
										</tr>
										<tr>
											<td style="width:130px;text-align: right;padding-top: 13px;">给药途径名称_英文：</td>
											<td><input type="text" name="DN_ENG" id="DN_ENG"  maxlength="255" value="${pd.ENG_NAME }" style="width:98%;"/></td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源-给药途径名称_英文：</td>
											<td><input type="text" name="ORG_DN_ENG" id="ORG_DN_ENG"  maxlength="255" value="${pd.ORG_ENG_NAME }" style="width:98%;"/></td>
													<input type="hidden" name="STAD_DN_ID" id="STAD_DN_ID"  value="${pd.STAD_DR_ID }"  maxlength="50"/><!-- 标准名称ID -->
										</tr>
										<tr>
											<td style="width:130px;text-align: right;padding-top: 13px;">给药途径名称_英文缩写：</td>
											<td><input type="text" name="FAC_ABB" id="FAC_ABB" maxlength="255" value="${pd.ORG_DR_ENG }" style="width:98%;"/></td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源-给药途径名称_英文缩写：</td>
											<td><input type="text" name="ORG_FAC_ABB" id="ORG_FAC_ABB"  value="${pd.ORG_DR_CHN_ABB }"  maxlength="255" style="width:98%;"/></td>
										</tr>
										<tr>
											<td style="width:130px;text-align: right;padding-top: 13px;">概念：</td>
											<td><input type="text" name="COUNTRY" id="COUNTRY" maxlength="255" value="${pd.CONCEPT }" style="width:98%;"/></td>
											<td style="width:130px;text-align: right;padding-top: 13px;">概念来源：</td>
											<td><input type="text" name="DISTRICT" id="DISTRICT"  value="${pd.CONCEPT_ORG }"  maxlength="255" style="width:98%;"/></td>
										</tr>
										<!-- 若新增的词语为同义词，则需要获取以下信息 -->
										<tr id="stad1">
											<td style="width:130px;text-align: right;padding-top: 13px;">标准给药途径名称_中文：</td>
											<td>
												<input type="text" name="STAD_CHN" id="STAD_CHN"  value="${pd.STAD_DR_CHN }"  maxlength="255" style="width:85%;" readonly/>
												<a onclick="selectDiag();">选择</a>	
											</td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源-标准给药途径名称_中文：</td>
											<td><input type="text" name="ORG_STAD_CHN" id="ORG_STAD_CHN" value="${pd.ORG_STAD_DR_CHN }" maxlength="255" title="备注" style="width:98%;" readonly/></td>
										</tr>
										<tr id="stad2">
											<td style="width:130px;text-align: right;padding-top: 13px;">标准给药途径名称_英文：</td>
											<td>
												<input type="text" name="STAD_ENG" id="STAD_ENG"  value="${pd.STAD_DR_ENG }" maxlength="255" style="width:98%;" readonly/>
											</td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源-标准给药途径名称_英文：</td>
											<td>
												<input type="text" name="ORG_STAD_ENG" id="ORG_STAD_ENG"  value="${pd.ORG_STAD_DR_ENG }" maxlength="255" style="width:98%;" readonly/>
											</td>
										</tr>
										<tr>
											...
											<td style="text-align: center;" colspan="10">
												<c:if test="${MSG=='saveDiagOsyn'}">
													...
													<c:if test="${pd.wordType==1 }">
														<input type="hidden" name="UPD_DESC" value="新增药品给药途径标准词"/>
													</c:if>
													<c:if test="${pd.wordType==0 }">
														<input type="hidden" name="UPD_DESC" value="新增药品给药途径同义词"/>
													</c:if>
												...
												<c:if test="${MSG=='edit'}">
													...
													<a class="btn btn-mini btn-primary" onclick="searchHist('${pd.DN_ID}','${pd.termCategory}');">查看历史记录</a>
												</c:if>
...
<script type="text/javascript">
	...
	function checkData(){
		...
		if($("#DN_CHN").val()==null ||$.trim($("#DN_CHN").val())=="" ){
			$("#DN_CHN").tips({ side:3, msg:'请输入给药途径中文', bg:'#AE81FF',  time:3   });
			flag = false;
		}
		if($("#DN_CHN").val()!=null ||$.trim($("#DN_CHN").val())!="" ){
		...
		if($("input[name='IS_DISABLE']:checked").val()==null ||$.trim($("input[name='IS_DISABLE']:checked").val())=="" ){
			$("input[name='IS_DISABLE']").tips({ side:3, msg:'请选择是否停用', bg:'#AE81FF',  time:3   });
			flag = false;
		}
		...
		if($("input[name='WORD_TYPE']:checked").val()==0){
			if($("#STAD_CHN").val()==null || $.trim($("input[name='IS_DISABLE']:checked").val())==""){
				$("#STAD_CHN").tips({ side:3, msg:'请选择标准给药途径', bg:'#AE81FF',  time:3   });
			}
		}
		return flag;
	}
