				...
				<div class="page-content">
					<div class="row">
						...
						<div id="zhongxin" style="padding-top: 13px;">
							...
							<input type="hidden" name="USE_ONTO" value="${pd.USE_ONTO}" id="osynType"/>
							<table style="margin-top:5px;">
								<tr>
									...
									<td style="padding-left:2px;padding-right:8px;" nowrap>
										更新日期：
										<input class="span10 date-picker" name="UPDATE_TIME" id="UPDATE_TIME"  value="${pd.UPDATE_TIME}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="更新开始日期"/>
										<input class="span10 date-picker" name="UPDATE_TIME_END" id="UPDATE_TIME_END"  value="${pd.UPDATE_TIME_END}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="更新结束日期"/>
									</td>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" nowrap>药品规格单位中文</th>
									<th class="center" nowrap>药品标准规格单位中文</th>
									<th class="center" nowrap>药品规格单位英文</th>
									<th class="center" nowrap>来源药品规格单位中文</th>
									<th class="center" nowrap>来源药品规格单位英文</th>
									...
									<c:forEach items="${resultList}" var="drug" varStatus="vs">
										...
										<tr>
											<td class='center' style="width: 30px;">
												<input type="radio" name='standIs' value="${drug.PSU_ID}" CN="${drug.PSU_CHN}" EN="${drug.PSU_ENG}" O_CN="${drug.ORG_PSU_CHN}" O_EN="${drug.ORG_PSU_ENG}"/>
											</td>
											<td class="center">${drug.PSU_CHN}</td>
											<td class="center">${drug.PSU_STAD_CHN}</td>
											<td class="center">${drug.PSU_ENG}</td>
											<td class="center">${drug.ORG_PSU_CHN}</td>
											<td class="center">${drug.ORG_PSU_ENG}</td>
...
function selectCont(){
	...
	var useOnto = $("#USE_ONTO").val();
	var mydocument = parent.$("#_DialogFrame_0")[0].contentWindow.document;
	if (useOnto==10202) {
		mydocument.getElementById("PSU_ID").value = osyn.val();
		mydocument.getElementById("STAD_PACK_SPEC_UNIT_CHN").value = osyn.attr("CN");
		mydocument.getElementById("STAD_PACK_SPEC_UNIT_ENG").value = osyn.attr("EN");
	} else {
