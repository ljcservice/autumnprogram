...
<body class="no-skin">
					...
					<div class="row">
						<div class="col-xs-12">
								<form action="osyn/saveDiagOsyn.do" name="diagOsynForm" id="diagOsynForm" method="post">
									...
									<input type="hidden" name="DN_ID" id="DN_ID" value="${pd.F_ID }"/>
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<tr>
											<td style="width:130px;text-align: right;padding-top: 13px;">药品剂型-中文：</td>
											<td><input type="text" name="DN_CHN" id="DN_CHN" maxlength="255" placeholder="输入药品名称中文" onblur="checkName(this,'DN_CHN');" value="${pd.F_CHN_NAME }" style="width:98%;"/></td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源-药品剂型-中文：</td>
											<td>
												<input type="text" name="ORG_DN_CHN" id="ORG_DN_CHN"  value="${pd.ORG_DOS_CHN }"  maxlength="255" placeholder="这里输入术语规范名称" style="width:98%;"/>
											</td>
										</tr>
										<tr>
											<td style="width:130px;text-align: right;padding-top: 13px;">药品剂型-英文：</td>
											<td><input type="text" name="DN_ENG" id="DN_ENG"  maxlength="255" placeholder="输入术语名称英文" value="${pd.F_ENG_NAME }" style="width:98%;"/></td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源-药品剂型-英文：</td>
											<td><input type="text" name="ORG_DN_ENG" id="ORG_DN_ENG"  maxlength="255" placeholder="输入术语名称英文" value="${pd.ORG_DOS_ENG }" style="width:98%;"/></td>
													<input type="hidden" name="STAD_DN_ID" id="STAD_DN_ID"  value="${pd.STAD_DOS_ID }"  maxlength="50"/><!-- 标准名称ID -->
										</tr>
										<!-- 若新增的词语为同义词，则需要获取以下信息 -->
										<tr id="stad1">
											<td style="width:130px;text-align: right;padding-top: 13px;">药品标准剂型_中文：</td>
											<td>
												<input type="text" name="STAD_CHN" id="STAD_CHN"  value="${pd.STAD_DOS_CHN }"  maxlength="255" style="width:85%;" readonly/>
												<a onclick="selectDiag();">选择</a>	
											</td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源-药品标准剂型_中文：</td>
											<td><input type="text" name="ORG_STAD_CHN" id="ORG_STAD_CHN" value="${pd.ORG_STAD_DOS_CHN }" maxlength="300" title="备注" style="width:98%;" readonly/></td>
										</tr>
										<tr id="stad2">
											<td style="width:130px;text-align: right;padding-top: 13px;">药品标准剂型_英文：</td>
											<td>
												<input type="text" name="STAD_ENG" id="STAD_ENG"  value="${pd.STAD_DOS_ENG }" maxlength="32" placeholder="输入术语来源名称-英文" style="width:98%;" readonly/>
											</td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源-药品标准剂型_英文：</td>
											<td>
												<input type="text" name="ORG_STAD_ENG" id="ORG_STAD_ENG"  value="${pd.ORG_STAD_DOS_ENG }" maxlength="32" placeholder="输入术语来源名称-英文" style="width:98%;" readonly/>
											</td>
										</tr>
										<tr>
											...
											<td style="text-align: center;" colspan="10">
												<c:if test="${MSG=='saveDiagOsyn'}">
													...
													<c:if test="${pd.wordType==1 }">
														<input type="hidden" name="UPD_DESC" value="新增药品剂型标准词"/>
													</c:if>
													<c:if test="${pd.wordType==0 }">
														<input type="hidden" name="UPD_DESC" value="新增药品剂型同义词"/>
													</c:if>
												...
												<c:if test="${MSG=='edit'}">
													...
													<a class="btn btn-mini btn-primary" onclick="searchHist('${pd.F_ID}','${pd.termCategory}');">查看历史记录</a>
												</c:if>
	...
	<script src="static/ace/js/bootbox.js"></script><!-- 确认窗口 -->
...
<script type="text/javascript">
	...
	function checkData(){
		...
		if($("#DN_CHN").val()==null ||$.trim($("#DN_CHN").val())=="" ){
			$("#DN_CHN").tips({ side:3, msg:'请输入药品剂型中文名', bg:'#AE81FF',  time:3   });
			flag = false;
		}
		if($("#DN_CHN").val()!=null ||$.trim($("#DN_CHN").val())!="" ){
		...
		if($("input[name='WORD_TYPE']:checked").val()==0){
			if($("#STAD_CHN").val()==null || $.trim($("input[name='IS_DISABLE']:checked").val())==""){
				$("#STAD_CHN").tips({ side:3, msg:'请选择标准药品剂型', bg:'#AE81FF',  time:3   });
			}
		}
		return flag;
	}
