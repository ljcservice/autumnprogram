					...
					<div class="row">
						<div class="col-xs-12">
								<form action="osyn/saveDiagOsyn.do" name="diagOsynForm" id="diagOsynForm" method="post">
									...
									<input type="hidden" name="DN_ID" id="DN_ID" value="${pd.PSU_ID }"/>
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<tr>
											<td width="19%" style="text-align: right;padding-top: 13px;">药品规格单位中文:</td>
											<td width="31%"><input type="text" name="DN_CHN" id="DN_CHN" maxlength="255" value="${pd.PSU_CHN }" onblur="checkName(this,'DN_CHN');" style="width:98%;"/></td>
											<td width="19%" style="text-align: right;padding-top: 13px;">来源药品规格单位中文:</td>
											<td width="31%">
												<input type="text" name="ORG_DN_CHN" id="ORG_DN_CHN" value="${pd.ORG_PSU_CHN }"  maxlength="255" style="width:98%;"/>
											</td>
										</tr>
										<tr>
											<td style="width:130px;text-align: right;padding-top: 13px;">药品规格单位英文:</td>
											<td><input type="text" name="DN_ENG" id="DN_ENG"  maxlength="255" value="${pd.PSU_ENG }" style="width:98%;"/></td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源药品规格单位英文:</td>
											<td><input type="text" name="ORG_DN_ENG" id="ORG_DN_ENG"  maxlength="255" value="${pd.ORG_PSU_ENG }" style="width:98%;"/></td>
													<input type="hidden" name="STAD_DN_ID" id="STAD_DN_ID"  value="${pd.PSU_STAD_ID }"  maxlength="255" style="width:70%;"/><!-- 标准名称ID -->
										</tr>
										<!-- 若新增的词语为同义词，则需要获取以下信息 -->
										<tr id="stad1">
											<td style="width:130px;text-align: right;padding-top: 13px;">药品标准规格单位中文:</td>
											<td>
												<input type="text" name="STAD_CHN" id="STAD_CHN"  value="${pd.PSU_STAD_CHN }"  maxlength="255" style="width:85%;" readonly/>
												<a onclick="selectDiag();">选择</a>	
											</td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源药品标准规格单位中文:</td>
											<td><input type="text" name="ORG_STAD_CHN" id="ORG_STAD_CHN" value="${pd.ORG_PSU_STAD_CHN }" maxlength="255" style="width:98%;" readonly/></td>
										</tr>
										<tr id="stad2">
											<td style="width:130px;text-align: right;padding-top: 13px;">药品标准规格单位英文:</td>
											<td>
												<input type="text" name="STAD_ENG" id="STAD_ENG"  value="${pd.PSU_STAD_ENG }" maxlength="255" style="width:98%;" readonly/>
											</td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源药品标准规格单位英文:</td>
											<td>
												<input type="text" name="ORG_STAD_ENG" id="ORG_STAD_ENG"  value="${pd.ORG_PSU_STAD_ENG }" maxlength="255" style="width:98%;" readonly/>
											</td>
										</tr>
										<tr>
											...
											<td style="text-align: center;" colspan="10">
												<c:if test="${MSG=='saveDiagOsyn'}">
													...
													<c:if test="${pd.wordType==1 }">
														<input type="hidden" name="UPD_DESC" value="新增药品包装规格单位标准词"/>
													</c:if>
													<c:if test="${pd.wordType==0 }">
														<input type="hidden" name="UPD_DESC" value="新增药品包装规格单位同义词"/>
													</c:if>
												...
												<c:if test="${MSG=='edit'}">
													...
													<a class="btn btn-mini btn-primary" onclick="searchHist('${pd.PSU_ID}','${pd.termCategory}');">查看历史记录</a>
												</c:if>
...
<script type="text/javascript">
	...
	function checkData(){
		...
		if($("#DN_CHN").val()==null ||$.trim($("#DN_CHN").val())=="" ){
			$("#DN_CHN").tips({ side:3, msg:'请输入药品规格单位', bg:'#AE81FF',  time:3   });
			flag = false;
		}
		if($("#DN_CHN").val()!=null ||$.trim($("#DN_CHN").val())!="" ){
		...
		if($("input[name='WORD_TYPE']:checked").val()==0){
			if($("#STAD_CHN").val()==null || $.trim($("input[name='IS_DISABLE']:checked").val())==""){
				$("#STAD_CHN").tips({ side:3, msg:'请选择药品标准规格单位', bg:'#AE81FF',  time:3   });
			}
		}
		return flag;
	}
	...
	function selectDiag(){
		...
		diag.Title ="标准包装规格单位";
		diag.URL = '<%=path%>/standard/osynStand.do?ontoType='+ontoType+'&use_onto='+ontoType;
		diag.Width = 1000;
	 	diag.Height = 600;
