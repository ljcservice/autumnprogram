									...
									<c:forEach items="${resultList}" var="drug" varStatus="vs">
										...
										<tr>
											<td class='center' style="width: 30px;">
												<input type="radio" name='standIs' value="${drug.DN_ID}" CN="${drug.CHN_NAME}" EN="${drug.ENG_NAME}" O_CN="${drug.ORG_CHN_NAME}" O_EN="${drug.ORG_ENG_NAME}"/>
											</td>
											<td class="center">${drug.CHN_NAME}</td>
											<td class="center">${drug.ENG_NAME}</td>
											<td class="center">${drug.STAD_CHN_NAME}</td>
											<td class="center">${drug.ORG_CHN_NAME}</td>
											<td class="center">${drug.ORG_ENG_NAME}</td>
											<td class="center">
												<c:if test="${drug.WORD_TYPE==10402 }">药品通用名</c:if>
												<c:if test="${drug.WORD_TYPE==10403 }">药品产品名 </c:if>
												<c:if test="${drug.WORD_TYPE==10404}">药品商品名</c:if>
												<c:if test="${drug.WORD_TYPE==10405 }">药品化学名</c:if>
											</td>
