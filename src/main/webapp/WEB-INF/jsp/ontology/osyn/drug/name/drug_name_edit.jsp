								...
								<form action="osyn/saveDiagOsyn.do" name="diagOsynForm" id="diagOsynForm" method="post">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										<tr>
											...
											<td width="19%" style="text-align: right;padding-top: 13px;">词语类型:</td>
											<td width="31%">
												<select class="chosen-select form-control" name="ONTO_TYPE" id="ONTO_TYPE" data-placeholder="选择药品名称类型" style="vertical-align:top;width: 98%;">
											 		<option value=""></option>
										...
										<tr>
											<td style="width:130px;text-align: right;padding-top: 13px;">药品中文名:</td>
											<td><input type="text" name="DN_CHN" id="DN_CHN" maxlength="255" placeholder="输入药品名称中文" onblur="checkName(this,'DN_CHN');" value="${pd.CHN_NAME }" style="width:98%;"/></td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源药品中文名:</td>
											<td>
												<input type="text" name="ORG_DN_CHN" id="ORG_DN_CHN"  value="${pd.ORG_CHN_NAME }"  maxlength="255" placeholder="这里输入术语规范名称" style="width:98%;"/>
											</td>
										</tr>
										<tr>
											<td style="width:130px;text-align: right;padding-top: 13px;">药品英文名:</td>
											<td><input type="text" name="DN_ENG" id="DN_ENG"  maxlength="255" placeholder="输入术语名称英文" value="${pd.ENG_NAME }" style="width:98%;"/></td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源药品英文名:</td>
											<td><input type="text" name="ORG_DN_ENG" id="ORG_DN_ENG"  maxlength="255" placeholder="输入术语名称英文" value="${pd.ORG_ENG_NAME }" style="width:98%;"/></td>
													<input type="hidden" name="STAD_DN_ID" id="STAD_DN_ID"  value="${pd.STAD_NAME_ID }"  maxlength="255" style="width:70%;"/><!-- 标准名称ID -->
										</tr>
										<!-- 若新增的词语为同义词，则需要获取以下信息 -->
										<tr id="stad1">
											<td style="width:130px;text-align: right;padding-top: 13px;">药品标准中文名:</td>
											<td>
												<input type="text" name="STAD_CHN" id="STAD_CHN"  value="${pd.STAD_CHN_NAME }"  maxlength="255" placeholder="这里输入术语规范名称" style="width:85%;" readonly/>
												<a onclick="selectDiag();">选择</a>	
											</td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源标准药品名称中文:</td>
											<td><input type="text" name="ORG_STAD_CHN" id="ORG_STAD_CHN" value="${pd.ORG_STND_CHN_NAME }" placeholder="输入来源药品名称英文" maxlength="300" title="备注" style="width:98%;" readonly/></td>
										</tr>
										<tr id="stad2">
											<td style="width:130px;text-align: right;padding-top: 13px;">药品标准英文名:</td>
											<td>
												<input type="text" name="STAD_ENG" id="STAD_ENG"  value="${pd.STAD_ENG_NAME }" maxlength="32" placeholder="输入术语来源名称-英文" style="width:98%;" readonly/>
											</td>
											<td style="width:130px;text-align: right;padding-top: 13px;">来源药品标准名称英文:</td>
											<td>
												<input type="text" name="ORG_STAD_ENG" id="ORG_STAD_ENG"  value="${pd.ORG_STND_ENG_NAME }" maxlength="32" placeholder="输入术语来源名称-英文" style="width:98%;" readonly/>
											</td>
										</tr>
										<tr>
											...
											<td style="text-align: center;" colspan="10">
												<c:if test="${MSG=='saveDiagOsyn'}">
													...
													<c:if test="${pd.wordType==1 }">
														<input type="hidden" name="UPD_DESC" value="新增药品名称标准词标准词"/>
													</c:if>
													<c:if test="${pd.wordType==0 }">
														<input type="hidden" name="UPD_DESC" value="新增药品名称同义词"/>
													</c:if>
...
<script type="text/javascript">
	...
	function checkData(){
		var flag = true;
		if($("#ONTO_TYPE").val()==null ||$.trim($("#ONTO_TYPE").val())==""){
			$("#ONTO_TYPE").tips({ side:3, msg:'请输入选择词语类型', bg:'#AE81FF',  time:3   });
			flag = false;
		}
		if($("#DN_CHN").val()==null ||$.trim($("#DN_CHN").val())=="" ){
			$("#DN_CHN").tips({ side:3, msg:'请输入药品中文名', bg:'#AE81FF',  time:3   });
			flag = false;
		}
		if($("#DN_CHN").val()!=null ||$.trim($("#DN_CHN").val())!="" ){
		...
		if($("input[name='WORD_TYPE']:checked").val()==0){
			if($("#STAD_CHN").val()==null || $.trim($("input[name='IS_DISABLE']:checked").val())==""){
				$("#STAD_CHN").tips({ side:3, msg:'请选择药品标准名称', bg:'#AE81FF',  time:3   });
			}
		}
		return flag;
	}
