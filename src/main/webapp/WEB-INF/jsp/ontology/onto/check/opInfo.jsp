...
<body class="no-skin">
					...
					<div class="row">
						...
						<div >
							<table class="table table-bordered table-hover">
								...
								<tr>
									<td align="right" width="24%">手术名称中文:</td><td width="38%">${oldOnto.STAD_DN_CHN}</td><td width="38%"  > ${newOnto.STAD_DN_CHN}<c:if test="${newOnto.OP_TYPE <=1}"><span class="edit"  onclick="setInput(this,'STAD_DN_CHN');" /></c:if></td>
								</tr>
								<tr>
									<td align="right">手术名称英文:</td><td>${oldOnto.STAD_DN_ENG}</td><td >${newOnto.STAD_DN_ENG}<c:if test="${newOnto.OP_TYPE <=1}"><span class="edit"  onclick="setInput(this,'STAD_DN_ENG');"/></c:if> </td>
								</tr>
								<tr>
									<td align="right">手术上级名称:</td><td>${oldOnto.PARENT_NAME}</td><td ><span id="newParent" >${newOnto.PARENT_NAME}</span><c:if test="${newOnto.OP_TYPE <=2}"> <span class="edit" onclick="selectCategory();"/> </c:if></td>
								</tr>
								<tr>
							...
							<div>
								...
								<c:when test="${not empty osynList}">
									<div style="height: 37px;font-size: 16px;"><span style="background-color: #6faed9;">&nbsp;新增手术同义词：</span></div>
									<div>
										...
										<table  class='table table-bordered table-hover'>
											<tr><td align="right">术语名称中文:</td><td class="memo">${osyn.DN_CHN}</td><td align="right">术语来源名称中文:</td><td class="memo">${osyn.ORG_DN_CHN}</td><td align="right">同义词类型:</td><td>
												...
												</td><td align="right">停用标记:</td><td><c:if test="${osyn.IS_DISABLE == '0' }">否</c:if><c:if test="${osyn.IS_DISABLE == '1' }">是</c:if></td></tr>
	...
	<script type="text/javascript" src="static/js/ontology/check.js"></script>
	</body>
</html>