			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<input type="hidden" id="query_task_stat" value="${pd.query_task_stat }"><!-- 查询的任务的状态 -->
						<input type="hidden" id="query_step" value="${pd.query_step }"><!-- 查询的任务的步骤 -->
						<input type="hidden" id="query_task_type_child_id" value="${pd.query_task_type_child_id }"><!-- 查询的任务的子类型 -->
						<form action="taskQuery/listTask.do" method="post" name="taskQueryForm" id="taskQueryForm">
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;"> 
									<select class="chosen-select form-control" name="TASK_TYPE_CHILD_ID" id="TASK_TYPE_CHILD_ID" data-placeholder="任务子类型"  style="vertical-align:top;width: 120px;" onchange="changeS();">
								...
								<td style="vertical-align:top;padding-left:2px;"> 
									<select class="chosen-select form-control" name="TASK_STAT" id="TASK_STAT" data-placeholder="任务状态"  style="vertical-align:top;width: 120px;">
										...
										<option value="2" <c:if test="${pd.TASK_STAT == '2' }">selected</c:if> >已提交</option>
										<!-- <option value="3" <c:if test="${pd.TASK_STAT == '3' }">selected</c:if> >已完成</option> -->
									</select>
								</td>
								<td style="vertical-align:top;padding-left:2px;"> 
									<span class="input-icon">
										<input style="width:150px" id="DUTY_USER" type="text" name="DUTY_USER" value="${pd.DUTY_USER }" placeholder="这里输入责任人" />
									</span>
								</td>
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;padding-top:2px;">
									...
									<ts:rights code="taskQuery/revokeTaskAll">
										<a class="btn btn-light btn-xs" onclick="task_revoke()"  title="撤销">
										<i id="nav-search-icon" class="ace-icon fa fa-file-text-o bigger-110 nav-search-icon blue"></i>撤销</a>
									</ts:rights>
									...
									<c:forEach items="${taskList}" var="t" varStatus="vs">
										<tr ondblclick="task_ctl(this);">
											...
											<td style="width: 60px;" class="center">
												...
												<c:if test="${t.TASK_STAT == '2' }"><span class="label label-success arrowed-in">已提交</span></c:if>
												<c:if test="${t.TASK_STAT == '3' }"><span class="label label-success arrowed-in">已完成</span></c:if>
											</td>
...
function task_ctr_diag(task_id,task_stat){
	 ...
	 diag.Drag=true;
	 diag.Title ="人工干预";
	 diag.URL = '<%=path%>/taskQuery/toCtl.do?task_id='+task_id+'&task_stat='+task_stat;
...
function task_submit(){
	...
	//校验已处理的状态才能够提交
	var query_task_stat=$("#query_task_stat").val();
	if(query_task_stat!=1){
		$("#nav-search-icon").tips({
			side:3,
            msg:'请查询"已处理"状态任务进行提交',
            bg:'#AE81FF',
            time:8
        });
		return;
...
//撤销
function task_revoke(){
	if($("#DUTY_USER_ID").val()!=''){
		bootbox.dialog({
			message: "<span class='bigger-110'>当前用户不具有撤销的权限</span>",
			buttons: 			
			{ "button":{ "label":"确定", "className":"btn-sm btn-success"}}
		});
		return;
	...
	//校验未处理的状态才能够撤销
	var query_task_stat=$("#query_task_stat").val();
	if(query_task_stat!=0){
		$("#nav-search-icon").tips({
			side:3,
            msg:'请查询"未处理"状态任务进行撤销',
            bg:'#AE81FF',
            time:8
        });
		return;
	...
	bootbox.confirm("确认要撤销所选择的任务吗?", function(result) {
		if(result) {
			var str = '';
			for(var i=0;i < document.getElementsByName('ids').length;i++)
			{
			...
			if(str==''){
				...
				var query_step=$("#query_step").val();
				var query_task_type_child_id=$("#query_task_type_child_id").val();
				$.ajax({
					type: "POST",
					url: '<%=basePath%>taskQuery/revokeTaskAll.do?tm='+new Date().getTime(),
			    	data: {TASK_IDS:str,query_step:query_step,query_task_type_child_id:query_task_type_child_id},
					dataType:'json',
					//beforeSend: validateData,
					cache: false,
					success: function(data){
