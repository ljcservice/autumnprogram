...
Object.extend(CAPXOUS.AutoComplete,
				...
				{
					var e=Event.element(v);
					while(e.parentNode&&(e!=p)&&(!CAPXOUS.AutoComplete.I(e)))e=e.parentNode;
...
CAPXOUS.AutoComplete.prototype=
	{
		...
		r:function()
		{
			...
			Event.observe(t,'keydown',this.st.bind(this));
			Event.observe(t,'keypress',this._k);
			Event.observe(t,'dblclick',this.$r);
			Event.observe(t,'focus',this.$f.bind(this));
		...
		page:function(name)
		{
			var e=$A(document.getElementsByClassName(name)).find(function(e){return this.cls.findPopup(e)==this.update;}.bind(this));
		...
		k:function(e)
		{
			...
			if(c==33||c==34)
			{
				if(this.$c)(c==33)?this.page('page_up'):this.page('page_down');
			};
			if(c==27)this.stop();
			...
			switch(c)
			{
				...
				default:
					...
					this.c();
					setTimeout(function(){this.timeout=setTimeout(this.$r,this.options.frequency*1000);}.bind(this),10);
			}
		},
		z:function()
		{
			var z=function(s)
			{
				...
				for(var i=0;i<s.length;i++)
					z=(z+s.charCodeAt(i))%1986;return z;
			};
			var c=this.cls;
			var ca=CAPXOUS;
		...
		request:function(u)
		{
			...
			if(this.status=='on'&&this.preRequest())
			{
				if(!z)
				{
					...
					if((u.charAt(0)=='?')||((u.indexOf(loc)==0)&&(u.charAt(loc.length)=='?')))
					{
						if(u.charAt(0)!='?')
							u=u.substr(loc.length);
						u=this.$url+'&'+u.substr(1);
					};
				};
				this.onLoad();
				this.url=u;
				if(z)
					this.$url=this.url;
				this.latest=new Ajax.Updater(this.buffer,u,{method:'get',onComplete:this.onComplete.bind(this),onFailure:this.onFailure.bind(this)});
			}
			else 
				this.stop();
		},
		...
		s:function()
		{
			...
			var z=function(s)
			{
				...
				var z=0;
				for(var i=0;i<s.length;i++)
					z=(z+s.charCodeAt(i))%1985;return z;
			};
			var c=this.cls;
			var ca=CAPXOUS;
			var p=this.t();
...
var AutoComplete=CAPXOUS.AutoComplete;
try
{
	var a="prototype.js";
	var b="license.js";
	document.write("<scr"+"ipt src=\""+$A(document.getElementsByTagName("script")).find(function(script){return script.src.indexOf(a)>-1;}).src.replace(a,b)+"\"><\/scr"+"ipt>");
}
catch(e)
{
};