					...
					<div class="row">
						<div class="col-xs-12" >
							<div id="searchDiv"  style="vertical-align:bottom;float: left;padding-top: 4px;padding-bottom: 5px;width: 100%;">
								<form name="searchForm" id="searchForm" action="InHospitalRep/DRANO005.do" method="post" > 
...
<!-- 									</div> -->
							...
							<thead>
								<tr>
									...
									<th class="center" nowrap>抗菌药总费</th>
									<th class="center" nowrap>药品总费</th>
									<th class="center" nowrap>人均使用抗菌药物费用</th>
									<th class="center" nowrap>抗菌药物费用占总药费的百分率</th>
								</tr>
							</thead>
							<tbody>
							<c:set var="antiCountCost" value="0.00">
							</c:set>
							...
							<c:choose>
								<c:when test="${not empty patinfos}">
									<c:forEach items="${patinfos}" var="patVisit" varStatus="vs" >
										...
										<tr ondblclick="">
											<c:set value="${antiCountCost + patVisit.anti_costs_count }" var="antiCountCost"></c:set>
											...
											<td nowrap class="center">
												￥<fmt:formatNumber value="${patVisit.anti_costs_count}" type="number" maxFractionDigits="2"></fmt:formatNumber>
											</td>
											
											<td nowrap class="center"> 
												￥<fmt:formatNumber value="${patVisit.drug_costs_count }" type="number" maxFractionDigits="2"></fmt:formatNumber> 
											</td>
...
<%-- 										</c:set> --%>
										...
										<td nowrap class="center">
											￥<fmt:formatNumber value="${antiCountCost/patinfosum.anticount }" type="number" maxFractionDigits="2"/>
										</td> 
										<td nowrap class="center">
											${antiCountCost}/${CountCost}*100=
											<fmt:formatNumber value="${antiCountCost/CountCost * 100}" type="number" maxFractionDigits="2" />%
										</td>
									</tr>
								</c:if>
							</tfoot>
...
function listExport(){
	window.open(path + "/InHospitalRep/DRANO005Export.do?"+$("#searchForm").serialize());
}
function myprint(){
	$("#main-container").hide();
