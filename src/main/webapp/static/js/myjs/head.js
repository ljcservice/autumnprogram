...
function getHeadMsg(){
	$.ajax({
		type: "POST",
		url: path+'/head/getList.do?tm='+new Date().getTime(),
    	data: encodeURI(""),
		dataType:'json',
...
function getFhsmsCount(){
	$.ajax({
		type: "POST",
		url: path+'/head/getFhsmsCount.do?tm='+new Date().getTime(),
    	data: encodeURI(""),
		dataType:'json',
		cache: false,
		success: function(data){
...
function editUserH(){
	 ...
	 diag.Drag=true;
	 diag.Title ="个人资料";
	 diag.URL = path+'/user/goEditMyU.do';
	 diag.Width = 469;
	 diag.Height = 465;
...
function editSys(){
	 ...
	 diag.Drag=true;
	 diag.Title ="系统设置";
	 diag.URL = path+'/head/goSystem.do';
	 diag.Width = 600;
	 diag.Height = 526;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function fhsms(){
	 ...
	 diag.Drag=true;
	 diag.Title ="站内信";
	 diag.URL = path+'/fhsms/list.do?STATUS=2';
	 diag.Width = 800;
	 diag.Height = 500;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function changeMenus(){
	//window.location.href=path+'?changeMenu=yes';
	$("#changeMenus").submit();
}
